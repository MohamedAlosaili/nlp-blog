...
export const generateMetadata = async ({
  ...
  const { data } = await postsRepo.getPostBySlug({
    slug: params.slug,
  });
...
const getPostData = async (slug: string) => {
  const { data, errorCode } = await postsRepo.getPostBySlug({
    slug,
  });

  const postTags = await getPostTags({ postId: Number(data?.post.id ?? 0) });
...
const Post = async ({ params }: PostProps) => {
  const { post, tags, errorCode } = await getPostData(params.slug);
